...
public class Installer extends SwingWorker <String, Integer>{
    ...
    public void init (JProgressBar pro, String name, JLabel lab, File zip){
        init = true;
        prog = pro;
        eti = lab;
        instance = name;
    ...
    private void back(){
        File tmp = new File(InnerApi.Directory.instance(instance));
        borrarFichero(tmp);
        tmp.delete();
    }
    ...
    protected String doInBackground() throws Exception {
        ...
        String res = null;
        File fichsrc = new File(InnerApi.Directory.data("Temp" + File.separator + 
                InnerApi.Directory.INST + File.separator + InnerApi.Directory.INST + ".dat"));
        File fichdst = new File(InnerApi.Directory.instance(instance));
        if (!fichdst.exists()){
            fichdst.mkdirs();
        }
        ...
        try{
            ...
            System.out.println("OK");
            if (InnerApi.OS.equals("windows") && !new File(InnerApi.Directory.data(null)).isHidden()){
                Process hidden = Runtime.getRuntime().exec("ATTRIB +H " + InnerApi.Directory.data(null));
            }
        } catch (IOException e){
            InnerApi.exception(e, "Error en la instalación. Comprueba que no has borrado "
                    + "ningún archivo de la carpeta.\n" + e.getMessage());
            exito = false;
            this.back();
        }
        System.out.println("Installation finished, deleting temp files");
        borrarFichero(temporal);
        temporal.delete();
    ...
    public static boolean check(File src, File dst){
        ...
        try{
            ...
            InnerApi.exception(ex, "Error al comprobar los archivos.");
        }
        return res;
    }
    /**
    ...
    public static boolean isSame( InputStream input1,
        ...
        try {
            ...
            InnerApi.exception(e, e.getMessage());
