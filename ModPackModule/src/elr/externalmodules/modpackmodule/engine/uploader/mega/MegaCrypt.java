...
/*    public static String base64_url_decode(String data) {
    ...
    public static String encrypt_attr(String attribute, long[] key){
        attribute = "MEGA" + attribute;
        try {
            return new String(aes_cbc_encrypt(attribute.getBytes("ISO-8859-1"), 
                    a32_to_str(key).getBytes("ISO-8859-1")), "ISO-8859-1");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
    ...
    public static byte[] aLong_to_aByte(long... longKey){
        byte[] buffer = new byte[longKey.length * 8];
        ByteBuffer bb = ByteBuffer.wrap(buffer);
        for (int i = 0; i < longKey.length; i++) {
            bb.putLong(longKey[i]);
        }
        return bb.array();
    }
    
    public static long[] aByte_to_aLong(byte[] key){
        ByteBuffer bb = ByteBuffer.wrap(key);
        long[] res = new long[key.length / 8];
        for (int i = 0; i < res.length; i++) {
            res[i] = bb.getLong(i * 8);
        }
        return res;
    }
